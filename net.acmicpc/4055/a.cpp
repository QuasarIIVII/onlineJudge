...
int main(){
	constexpr uf4 TN = 1<<20;
	qio_init((6*100 + 4)*TN+64, 64*TN+64);
	qio_is_load();

	for(uf4 t = 1;; ++t){
		uf1 n;
		if(!(n = qio_is_read_iu8<uf4>())) break;
		using T = pair<uf1, uf1>;
		priority_queue<T, varray<T, 129>, greater<T>> pq;
		while(n--){
			uf1 a, b;
			a = qio_is_read_iu8<uf1>();
			b = qio_is_read_iu8<uf1>();
			pq.emplace(a*2, b*2);
		}
		uf1 r = 0;
		for(uf1 p = 0; pq.size(); pq.pop()){
			const auto &[x, y] = pq.top();
			DEBUG cout<<+p<<' '<<+y<<' '<<+x<<"\n";
			if(p+1 <= y) ++r, p = p<x ? x+1 : p+1;
		}
		qio_os_write_cstr("On day ");
		qio_os_write_u8(t);
		qio_os_write_cstr(" Emma can attend as many as ");
		qio_os_write_u8(r);
		qio_os_write_cstr(" parties.\n");
	}
