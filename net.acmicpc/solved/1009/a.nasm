...
; esi : exponent
f:
	imul edx, edi, 52429 ; edx = edi * round(2^19 / 10.)
	shr edx, 19 ; edx = edi / 10
	lea edx, [edx + edx*4] ; edx = 5 * edx
	lea edx, [edx+edx] ; edx = 2 * edx
	sub edi, edx ; edi = edi - edx ; edi = edi % 10
	...
	.l0s:
		...
		.l0.c0s:
			mul edi ; eax *= edi
			imul edi, edi ; edi *= edi
			imul edx, eax, 52429 ; edx = eax * round(2^19 / 10.)
			imul ecx, edi, 52429 ; ecx = edi * round(2^19 / 10.)
			shr edx, 19 ; edx = eax / 10
			shr ecx, 19 ; ecx = edi / 10
			lea edx, [edx + edx*4] ; edx *= 5
			lea ecx, [ecx + ecx*4] ; ecx *= 5
			lea edx, [edx+edx] ; edx *= 2
			lea ecx, [ecx+ecx] ; ecx *= 2
			sub eax, edx ; eax = eax - edx ; eax = eax % 10
			sub edi, ecx ; edi = edi - ecx ; edi = edi % 10
