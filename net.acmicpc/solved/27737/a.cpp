...
int main(){
	qio_init(2*10000 + 64, 64);
	qio_is_load();

	u1 n = qio_is_read_iu8<u1>();
	u4 m = qio_is_read_iu8<u4>();
	const u4 M = m;
	u4 k = qio_is_read_iu8<u4>();

	array<array<u1, 128>, 128> a;

	for(u1 i=n; i--;)for(u1 j=n; j--;)
		a[i][j] = qio_is_read_iu8<u1>();
	...
	for(u1 i=n; i--;)for(u1 j=n; j--;){
		...
		if(r){
			if(const u4 d = (r-1u)/k + 1u; m < d){
				qio_os_write_cstr("IMPOSSIBLE");
				qio_flush();
				qio_close();
				return 0;
			}else m -= d;
