...
int main(){
	qio_init(6*100*300+64, 4*300+64);
	qio_is_load();

	uf2 n = qio_is_read_iu8<uf2>();
	uf2 m = qio_is_read_iu8<uf2>();

	using T=pair<uf4, uf2>;
	priority_queue<T, varray<T, 1<<15>, greater<T>> pq;
	for(uf2 i=n; i--;){
		uf4 s=0;
		// for(uf2 j=0; j<m; ++j)
		for(uf2 j=m+1; --j;)
			pq.emplace(s+=qio_is_read_iu8<uf4>(), j);
	}
	...
	while(pq.size()){
		const uf2 i = m-pq.top().second;
		DEBUG cout<<pq.top().first<<" "<<i<<endl;
		pq.pop();
		++a[i+1], --a[i];
