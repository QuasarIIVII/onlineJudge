...
int main(){
	qio_init(100'000*(5+5+7)+64, 64);
	qio_is_load();

	qio_is_read_iu8<uf2>();

	array<vector<pair<uf2, uf4>>, 1000> a;

	for(uf4 m = qio_is_read_iu8<uf4>(); m--;){
		uf2 u = qio_is_read_iu8<uf2>()-1;
		uf2 v = qio_is_read_iu8<uf2>()-1;
		uf4 w = qio_is_read_iu8<uf4>();

		a[u].emplace_back(v, w);
	}

	const uf2 s = qio_is_read_iu8<uf2>()-1;
	const uf2 e = qio_is_read_iu8<uf2>()-1;

	array<uf4, 1000> d;
	d.fill(numeric_limits<uf4>::max());
	using T = pair<uf4, uf2>;
	constexpr auto f = [](const T& a, const T& b){ return a.first > b.first; };
	priority_queue<T, varray<T, 1<<17>, decltype(f)> pq(f);
	pq.emplace(0, s);
