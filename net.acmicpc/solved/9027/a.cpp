...
int main(){
	constexpr u4 T = 100;
	qio_init(T*(7+100'000*(10+6)) + 64, T*10 + 64);
	qio_is_load();

	for(u4 T=qio_is_read_iu8<u4>(); T--;){
		...
		q[n] = 0;
		for(u4 i=n; i--;)
			d[i] = qio_is_read_iu8<u4>();
		for(u4 i=n, qa=0; i--;){
			a[i] = qio_is_read_iu8<u2>();
			q[i] = q[i+1] + u8(d[i]-d[i+1])*qa;
			qa += a[i];
			DEBUG cout<<"qa="<<qa<<endl;
		}
		DEBUG for(u4 i=0; i<n; ++i)
			cout<<"i="<<i<<" a="<<a[i]<<" d="<<d[i]<<" q="<<q[i]<<endl;

		u4 x = 0, pa = 0;
		u8 r = numeric_limits<u8>::max(), p = 0;
		for(u4 i=0; i<n; ++i){
			DEBUG cout<<"i="<<i<<" p="<<p<<" q="<<q[i]<<" v="<<p+q[i]<<endl;
			if(const u8 v = p + q[i]; v <= r) r=v, x=i;
			pa += a[i];
			p += u8(d[i]-d[i+1])*pa;
		}
		qio_os_write_u8(d[x]);
