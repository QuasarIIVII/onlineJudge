...
public:
	...
	T operator[](size_t p) const { return a[p+N]; }
};
...
int main(){
	qio_init(100'000*(11+2+7+11) + 64, 100'000*11 + 64);
	qio_is_load();
	...
	a.resize(n);
	for(u4 i=0; i<n; ++i)
		a[i] = qio_is_read_iu8<u4>();

	ST<u4, 100'000, decltype(static_cast<const u4&(*)(const u4&, const u4&)>(min<u4>))> st(
		a,
		numeric_limits<u4>::max(),
		min<u4>
	);
