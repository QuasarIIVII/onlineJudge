...
int main(){
	qio_init(500'000*(7*2 + 1) + 64, 500'001+64);
	qio_is_load();
	...
	asm(R"(
	movq ql.io.data(%%rip),%%rbx
	movq ql.io.is.offset+0x08(%%rbx),%%rcx
	movq %%rcx,%[src]
	.Ql0s:
		movzbl (%%rcx),%%eax
		testb $0x40,%%al
	...
	.Ql0e:
	movb $0,-1(%%rcx)
	movq %%rcx,ql.io.is.offset+0x08(%%rbx)
	)"
	:[src]"=g"(s)
	:
	...
	array<vector<u4>, 500'000> a;
	for(uf4 i=n-1; i--;){
		uf4 p = qio_is_read_iu8<uf4>();
		uf4 q = qio_is_read_iu8<uf4>();
		DEBUG cout<<"edge: "<<p-1<<" "<<q-1<<endl;
		a[p-1].emplace_back(q-1);
		a[q-1].emplace_back(p-1);
	}

	array<char, 500'002> _d{};
	auto d = _d.begin()+1;
	u4 pd = 0;
	d[-1] = s[0];

	// lv, cur, parent
	queue<tuple<u4, u4, u4>> Q;

	for(Q.emplace(0, 0, 0); Q.size(); Q.pop()){
		const auto &[lv, q, p] = Q.front();
		if(s[p] != _d[lv]
		|| s[q] < d[lv]) continue;

		d[pd = lv] = s[q];
		for(const auto &nq : a[q]){
			if(nq == p) continue;
			Q.emplace(lv+1, nq, q);
		}
	}
