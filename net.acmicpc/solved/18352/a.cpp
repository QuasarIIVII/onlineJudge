...
int main(){
	qio_init(1'000'000*(7+7) + 64, 300'000*7 + 64);
	qio_is_load();

	qio_is_read_iu8<u4>();
	const u4 m = qio_is_read_iu8<u4>();
	const u4 k = qio_is_read_iu8<u4>()+1;
	const u4 x = qio_is_read_iu8<u4>()-1;

	array<vector<u4>, 300'000> e;
	for(u4 i=m; i--;){
	...
	u4 rn = 0;
	array<u4, 300'000> r;
	static array<u4, 300'000> d;
	u4 qp = 0, qq = 0;
	array<u4, 1'000'000> Q;
	Q[qq++] = x;
	...
	sort(r.begin(), r.begin()+rn, greater<u4>());
	for(u4 i=rn; i--;)
		qio_os_write_u8(r[i]+1),
		qio_os_write_cstr("\n");
