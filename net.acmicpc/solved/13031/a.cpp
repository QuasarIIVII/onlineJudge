...
int main(){
	...
	array<list<pair<u4, u4>>, 100'000> e;
	for(u4 i=1; i<n; ++i){
		const u4 v = qio_is_read_iu8<u4>();
		const u4 w = qio_is_read_iu8<u4>();
		e[v-1].emplace_back(i, w);
	}
	...
	const function<u4(const u4)> g = [&](const u4 u){
		u4 r = 0;
		array<u4, 100'000> Q;
		auto p = Q.begin(), q = Q.begin();
		*q++ = u;
		while(p != q){
			const u4 u = *p++;
			r += 1;
	...
	const function<u4(const u4, const u4)> f = [&](const u4 u, const u8 x){
		u4 r = 0;
		if(a[u] < x) return g(u);
