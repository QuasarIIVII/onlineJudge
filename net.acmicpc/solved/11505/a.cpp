...
int main(){
	...
	qio_init(
		1'000'000*11 + 10'000*(2+8+11) + 10'000*(2+8+8) + 64,
		11*10'000 + 64
	);
	...
	constexpr auto f = [](uf4 a, uf4 b){
		return static_cast<uf8>(a)*b % 1'000'000'007;
	};

	ST<uf4, 1'000'000, decltype(f)> st(
		[&N]{
			varray<uf4, 1'000'000> init;
			for(uf4 i=0; i<N; ++i)
				init.push_back(qio_is_read_iu8<uf4>());
			return init;
		}(),
		1,
		f
