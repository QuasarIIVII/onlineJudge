...
public:
	...
	ST(const varray<T, _N> &arr, const T &id, const F &f)
	: id(id), f(f){
		a.fill(id);
		size_t i = N;
		for(const T &v : arr) a[i++] = v;
...
int main(){
	qio_init(100'000*(11 + 2+7+11) + 64, 100'000*7 + 64);
	qio_is_load();

	const u4 n = qio_is_read_iu8<u4>();
	varray<u8, 100'000> va;
	va.resize(n);

	for(u4 i=0; i<n; ++i)
		va[i] = qio_is_read_iu8<u8>()&1 ? 1 : 0x1'0000'0000;

	ST<u8, 100'000, plus<u8>> st(va, 0, plus<u8>{});

	for(u4 Q=qio_is_read_iu8<u4>(); Q--;){
		const u4 c = qio_is_read_iu8<u4>();
		const u4 p = qio_is_read_iu8<u4>();
		const u4 q = qio_is_read_iu8<u4>();
		if(c&2){
			const u8 r = st.q(p-1, q);
			qio_os_write_u8(c&1 ? r&0xffff'ffff : r>>32);
			qio_os_write_cstr("\n");
		}else st.x(p-1, q&1 ? 1 : 0x1'0000'0000);
	}
