...
int main(){
	...
	for(uf4 i=1; i<=n+1; ++i){
		...
		DEBUG cout<<i<<' '<<it - b.begin()<<'\n';
		DEBUG{
			for(uf4 j=0; j<(be-b.begin()) || (cout<<endl, 0); ++j)
				cout<<b[j].first<<' ';
			for(uf4 j=0; j<(be-b.begin()) || (cout<<endl, 0); ++j)
				cout<<b[j].second<<' ';
			cout<<endl;
	...
	DEBUG{
		for(uf4 i=0; i<=n+1 || (cout<<endl, 0); ++i)
			cout<<a[i].first<<' ';
		for(uf4 i=0; i<=n+1 || (cout<<endl, 0); ++i)
			cout<<a[i].second<<' ';
	}
	...
	if(c > 3){
		qio_os_write_cstr("NO");
		qio_flush();
		qio_close();
		return 0;
	}

	DEBUG for(uf1 i=c; i-- || (cout<<endl, 0);)
		cout<<r[i]<<' ';

	qio_os_write_cstr("YES\n");
	qio_os_write_u8(c);
	qio_os_write_cstr("\n");
	...
	while(c--){
		qio_os_write_u8(r[c]);
		qio_os_write_cstr(" ");
		qio_os_write_i8(a[r[c]].first = a[r[c]-1].first);
		qio_os_write_cstr("\n");
	}
