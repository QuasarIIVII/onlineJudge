...
int main(){
	...
	array<pair<unordered_set<u4>, char[8]>, 100'000> e;

	for(u4 i=n; --i;){
		const u4 a = qio_is_read_iu8<u4>()-1;
		const u4 b = qio_is_read_iu8<u4>()-1;
		e[a].first.emplace(b);
		e[b].first.emplace(a);
	}

	if(qio_is_read_iu8<u4>()!=1){
		qio_os_write_cstr("0");
		qio_flush(), qio_close();
		return 0;
	}
	...
	u4 sp = 0;
	array<u4, 100'001> _st;
	u4 *st = _st.data()+1;
	_st[0] = numeric_limits<u4>::max();
	st[sp++] = 0;
	while(sp){
		DEBUG cout<<sp<<' '<<flush;
		const u4 u = st[sp-1];
		switch(e[u].first.size()){
		case 0:{
			DEBUG cout<<"0 "<<u+1<<endl;
			--sp;
			break;
		}
		case 1:{
			const u4 v = *e[u].first.begin();
			DEBUG cout<<"1 "<<u+1<<" "<<v+1<<' '<<*(st+sp-2)<<endl;
			if(v == *(st+sp-2)){
				--sp;
				break;
			}
		...
		default:{
			const u4 v = qio_is_read_iu8<u4>()-1;
			DEBUG cout<<"? "<<u+1<<" "<<v+1<<endl;
			const auto it = e[u].first.find(v);
			if(it == e[u].first.end()){
				qio_os_write_cstr("0");
				qio_flush(), qio_close();
				return 0;
