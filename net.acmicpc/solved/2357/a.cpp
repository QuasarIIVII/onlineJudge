...
int main(){
	...
	qio_init(1'100'000 + 2*700'000 + 64, 11*2*100'000 + 64);
	qio_is_load();

	uf4 n = qio_is_read_iu8<uf4>();
	uf4 q = qio_is_read_iu8<uf4>();

	varray<uf4, 100'000> a;
	for(uf4 i=0; i<n; ++i)
		a.push_back(qio_is_read_iu8<uf4>());

	// min segment tree
	using F = const uf4&(*)(const uf4&, const uf4&);
	ST<uf4, 100'000, F> stmn(
		a,
		numeric_limits<uf4>::max(),
		static_cast<F>(min<uf4>)
	), stmx(
		a,
		0,
		static_cast<F>(max<uf4>)
	);

	while(q--){
		uf4 l = qio_is_read_iu8<uf4>() - 1, r = qio_is_read_iu8<uf4>();
		qio_os_write_u8(stmn.q(l, r));
		qio_os_write_cstr(" ");
		qio_os_write_u8(stmx.q(l, r));
		qio_os_write_cstr("\n");
	}
