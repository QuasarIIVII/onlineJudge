...
int main(){
	qio_init(64, 4*(1<<20)+64);
	qio_is_load();

	u2 n = qio_is_read_iu8<u2>();

	const function<void(u2, u1, u1)> f = [&f](u2 n, u1 src, u1 dst){
		if(n==0) return;
		constexpr array<const char*, 10> a = {
			"", "\n1 2", "\n1 3", "",
			"\n2 1", "", "\n2 3", "",
			"\n3 1", "\n3 2",
		};
		u1 m = 3u - src - dst;
		f(n-1u, src, m);
		qio_os_write_cstr(a[src*4u | dst]);
		f(n-1u, m, dst);
	};

	qio_os_write_u8((1u<<n)-1u);
	f(n, 0, 2);
