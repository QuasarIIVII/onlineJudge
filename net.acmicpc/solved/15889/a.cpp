...
int main(){
	qio_init(64+30'000*8*2, 64);
	qio_is_load();

	uf2 n = qio_is_read_iu8<uf2>();
	array<pair<uf4, uf4>, 30'000> a;

	for(uf2 i=n; i--;) a[i].first = qio_is_read_iu8<uf4>();
	for(uf2 i=n; --i;) a[i].second = qio_is_read_iu8<uf4>();
	a[0].second = 0;
	...
	uf4 q = 0;
	uf2 i;
	for(i=0; i<n && a[i].first<=q; ++i){
		if(uf4 d = a[i].second + a[i].first; q < d)
			q = d;
	}

	i==n
	? qio_os_write_cstr("권병장님, 중대장님이 찾으십니다")
	: qio_os_write_cstr("엄마 나 전역 늦어질 것 같아");
