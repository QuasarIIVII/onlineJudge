...
int main(){
	qio_init(250*499*8 + 500*10 + 64, 4*500*500 + 64);
	qio_is_load();

	const u4 n = qio_is_read_iu8<u4>();
	array<set<u2>, 500> a;

	for(u4 m = qio_is_read_iu8<u4>(); m--;){
		const u4 x = qio_is_read_iu8<u4>()-1;
		const u4 y = qio_is_read_iu8<u4>()-1;
		a[x].emplace(y), a[y].emplace(x);
	}

	for(u4 Q = qio_is_read_iu8<u4>(); Q--;){
		const u4 c = qio_is_read_iu8<u4>();
		const u4 x = qio_is_read_iu8<u4>()-1;
		const u4 y = qio_is_read_iu8<u4>()-1;

		if(c==1) a[x].emplace(y), a[y].emplace(x);
		else a[x].erase(y), a[y].erase(x);

		u4 qp=0, qq=0;
		array<u2, 512> b, q;
		memset(b.data(), 0xff, sizeof(b));
		...
		for(u4 i=0; i<n || (qio_os_write_cstr("\n"), 0); ++i){
			qio_os_write_i8(static_cast<i2>(b[i])),
			qio_os_write_cstr(" ");
		}
	}
