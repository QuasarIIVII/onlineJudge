...
int main(){
	...
	priority_queue<if2, varray<if2, 131'073>, less<if2>> mxq;
	priority_queue<if2, varray<if2, 131'073>, greater<if2>> mnq;
	...
	for(;n--;){
		...
		if(!mnq.empty() && mxq.top() > mnq.top()){
			if2 a = mxq.top(); mxq.pop();
			if2 b = mnq.top(); mnq.pop();
			mxq.push(b);
			mnq.push(a);
		}

		DEBUG{
			cout<<mxq.size()<<" "<<mnq.size()<<"\t";
			cout<<mxq.top()<<' '<<(mnq.size() ? mnq.top() : 0)<<"\n";
		}

		qio_os_write_i8(mxq.top());
		qio_os_write_cstr("\n");
	}
