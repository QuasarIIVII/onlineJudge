...
int main(){
	qio_init(200'000*12+64, 64);
	qio_is_load();

	const u4 n = qio_is_read_iu8<u4>();
	const u4 k = qio_is_read_iu8<u4>();

	u4 pa=0, pb=0;
	array<u4, 200'000> a, b;

	for(u4 i=n; i--;){
		const u4 t = qio_is_read_iu8<u4>();
		if(t & 0x8000'0000) b[pb++] = -t;
		else a[t ? pa++ : pa] = t;
	}

	sort(a.begin(), a.begin()+pa);
	sort(b.begin(), b.begin()+pb);

	u8 r = 0;

	u4 i;
	if(pa){
		for(i=pa-1; k<=i ; i-=k) r += a[i]*2;
		r += a[i]*2;
	}
	if(pb){
		for(i=pb-1; k<=i ; i-=k) r += b[i]*2;
		r += b[i]*2;
	}
