...
int main(){
	...
	qio_init(5*500'000 + 7*500'000 + 64, 500'000*10 + 64);
	qio_is_load();

	uf4 n = qio_is_read_iu8<uf4>();
	uf4 m = qio_is_read_iu8<uf4>();

	varray<uf4, 500'000> a;
	for(uf4 i=0; i<n; ++i)
		a.push_back(qio_is_read_iu8<uf4>());

	ST<uf4, 500'000, plus<uf4>> st(a, 0, plus<uf4>());

	while(m--){
		uf4 p = qio_is_read_iu8<uf4>();
		qio_os_write_u8(st.q(0, p));
		st.x(p-1, 0);
		qio_os_write_cstr(" ");
	}
