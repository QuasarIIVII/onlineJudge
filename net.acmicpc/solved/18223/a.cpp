...
public:
	...
	constexpr typename varray::const_iterator end() const noexcept { return this->cbegin()+n; }
	...
	constexpr void emplace_back(Args&&... args) noexcept {
		this->operator[](n++) = T(forward<Args>(args)...);
	}
...
int main(){
	qio_init(10'000*(5+5+6)+64, 64);
	qio_is_load();

	array<vector<pair<u2, u2>>, 5'000> a;
	array<u4, 5'000> d;
	array<u1, 5'000> vv{};
	d.fill(0xffff'ffff);

	const u2 nv = qio_is_read_iu8<u2>();
	const u2 ne = qio_is_read_iu8<u2>();
	const u2 z = qio_is_read_iu8<u2>()-1;

	for(u2 i=ne; i--;){
		const u2 u = qio_is_read_iu8<u2>();
		const u2 v = qio_is_read_iu8<u2>();
		const u2 w = qio_is_read_iu8<u2>();
		a[u-1].emplace_back(v-1, w);
		a[v-1].emplace_back(u-1, w);
	}

	using TPQ = pair<u4, u2>;
	priority_queue<TPQ, varray<TPQ, 8192>, greater<TPQ>> pq;
	pq.emplace(0, 0);

	while(pq.size()){
		const auto [w, p] = pq.top();
		pq.pop();
		...
		DEBUG cout<<p<<' '<<w<<' '<<+vv[p]<<endl;

		for(const auto &[q, dw] : a[p]){
			const u4 ww = d[p] + dw;
			if(d[q] < ww) continue;
			vv[q] = d[q]!=ww ? vv[p] : vv[p]|vv[q];
			d[q] = ww;
			pq.emplace(ww, q);
		}
	}

	if(vv[nv-1]) qio_os_write_cstr("SAVE HIM");
	else qio_os_write_cstr("GOOD BYE");
