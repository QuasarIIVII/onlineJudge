...
int main(){
	...
	function<pair<uf4, uf4>(uf4)> f = [&](uf4 p){
		v.set(p);
		uf4 r0=0, r1=1;
		for(const uf4 &q : a[p]){
			if(v.test(q)) continue;
			const auto [f0, f1] = f(q);
			r0 += f1;
			r1 += min(f0, f1);
		}

		return pair<uf4, uf4>{r0, r1};
	};

	qio_os_write_u8([](const auto &a){return min(a.first, a.second);}(f(0)));
	qio_flush();
	qio_close();
	return 0;
}
