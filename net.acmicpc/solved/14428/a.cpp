...
int main(){
	qio_init(100'000*11 + 100'000*(2+7+11) + 64, 100'000*7 + 64);
	qio_is_load();
	...
	a.resize(n);
	for(u4 i=0; i<n; ++i)
		a[i] = {qio_is_read_iu8<u4>(), i+1};

	ST<T, 100'000, decltype(
		static_cast<const T&(*)(const T&, const T&)>(min<T>)
	)> st(
		a,
		{numeric_limits<u4>::max(), numeric_limits<u4>::max()},
		min<T>
	);
	...
	for(u4 Q = qio_is_read_iu8<u4>(); Q--;){
		...
		if(c&1) st.x(p-1, {q, p});
			...
			qio_os_write_u8(st.q(p-1, q).second),
			qio_os_write_cstr("\n");
	}
