...
int main(){
	qio_init(100'000*(2+8+12) + 64, 100'000*17 + 64);
	qio_is_load();

	ST<i8, 1'000'000, plus<>> st({}, 0, plus<>{});
	qio_is_read_iu8<u4>();

	for(u4 Q = qio_is_read_iu8<u4>(); Q--;){
		const u1 c = qio_is_read_iu8<u1>();
		const u4 p = qio_is_read_iu8<u4>();
		const u4 q = qio_is_read_iu8<u4>();

		if(c&1) st.x(p, st[p] + static_cast<i4>(q));
		else
			qio_os_write_i8(st.q(p, q+1)),
			qio_os_write_cstr("\n");
	}
