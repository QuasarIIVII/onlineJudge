...
int main(){
	qio_init(12*100'000 + 64, 100'000*12);
	qio_is_load();

	u4 n = qio_is_read_iu8<u4>();
	priority_queue<pair<i4, i4>, varray<pair<i4, i4>, 1<<17>, greater<pair<i4, i4>>> pq;
	while(n--){
		const i4 x = qio_is_read_iu8<i4>();
		if(x) pq.emplace(x<0 ? -x : x, x);
		else{
			if(pq.empty()) qio_os_write_cstr("0\n");
			else{
				qio_os_write_i8(pq.top().second);
				qio_os_write_cstr("\n");
				pq.pop();
			}
		}
