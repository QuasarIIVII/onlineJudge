...
int main(){
	qio_init(8*1'000'000+64, 8*1'000'000+64);
	qio_is_load();

	uf4 n = qio_is_read_iu8<uf4>();
	array<uf4, 1'000'001> a, r;
	a[0] = numeric_limits<uf4>::max();

	uf4 sp = 0;
	array<uf4, 1'000'000> st;

	for(uf4 i=n+1; --i;)
		a[i] = qio_is_read_iu8<uf4>();

	for(uf4 p=n+1; p--;){
		while(sp && a[st[sp-1]] < a[p])
			r[st[--sp]-1] = a[p];
		st[sp++] = p;
	}

	for(uf4 i=n; i--;)
		qio_os_write_i8(bit_cast<if4>(r[i])),
		qio_os_write_cstr(" ");
