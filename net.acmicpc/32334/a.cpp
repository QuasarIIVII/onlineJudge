...
int main(){
	...
	uf2 d=qio_is_read_iu8<uf2>();

	array<array<uf4, 1001>, 1001> a{};
	array<bitset<1001>, 1001> b;

	for(uf2 i=1; i<=n; ++i)for(uf2 j=1; j<=n; ++j){
		uf4 v = qio_is_read_iu8<uf4>();
		b[i][j] = v;
		a[i][j] = v + a[i-1][j] + a[i][j-1] - a[i-1][j-1];
	}

	uf4 r = numeric_limits<uf4>::max();
	uf2 x=0, y=0;
	for(uf2 i=n+1; --i;)for(uf2 j=n+1; --j;){
		if(b[i].test(j)) continue;

		uf2 pe = min(n, i+d), qe = min(n, j+d);
		uf2 ps = i>d ? i-d : 1, qs = j>d ? j-d : 1;
		uf4 v = a[pe][qe] - a[ps-1][qe] - a[pe][qs-1] + a[ps-1][qs-1];
